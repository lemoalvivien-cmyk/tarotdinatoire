...
export default defineConfig(({ mode }) => {
  ...
  // Security headers — ONLY applied in production to avoid blocking Lovable's iframe preview
  const prodHeaders = isProd
    ? {
        "Content-Security-Policy": [
          ...
          // frame-ancestors: allow only the site itself in production (no 'none' — that was blocking previews)
        ...
        // NOTE: X-Frame-Options DENY removed — it blocked Lovable preview iframe
      }
    : {};
    // In dev/preview: NO iframe-blocking headers so Lovable preview works
