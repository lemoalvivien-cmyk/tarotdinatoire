...
export function useKarma() {
  ...
  const awardMutation = useMutation<AwardResult, Error, KarmaAction>({
    mutationFn: async (action: KarmaAction) => {
    ...
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: ['karma', user?.id] });

      // Toast for XP gained
      if (result.xp_gained > 0) {
        toast(`+${result.xp_gained} XP`, {
          description: result.new_achievements.length > 0
            ? `🏆 Succès débloqué : ${result.new_achievements.map(k => ACHIEVEMENT_META[k]?.label ?? k).join(', ')}`
            : `Niveau ${result.level_name}`,
          duration: result.new_achievements.length > 0 ? 6000 : 3000,
          icon: result.new_achievements.length > 0 ? '🏆' : '✨',
        });
      }
    },
    onError: () => {
      // Silent fail — karma is a nice-to-have
    },
  });
  ...
  return {
    ...
    levelMeta: karma ? LEVEL_META[karma.level] ?? LEVEL_META[1] : LEVEL_META[1],
  };
}
